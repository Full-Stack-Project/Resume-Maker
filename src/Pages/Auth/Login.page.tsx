  ...
  const { handleSubmit, errors, touched, isSubmitting, getFieldProps } =
    useFormik({
      ...
      validationSchema: yup.object().shape({
        ...
        password: yup.string().required("Cannot login without a password"),
      }),
      onSubmit: (data) => {
        console.log(data);
      },
      ...
      <div className="px-10 sm:px-0 flex flex-col justify-center min-h-screen space-y-16 mx-auto max-w-26rem">
        <div className="space-y-3">
          <h1 className="text-4xl font-normal">
            Log In to{" "}
            <span className="text-primary-medium font-bold">GLA Resume</span>
          </h1>
          <h5 className="text-sm font-bold">
            New Here?{" "}
            <LinkTo to="/signup" className="border-b border-primary-medium">
              Create an account
            </LinkTo>
          </h5>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6" method="POST">
          ...
          <div className="flex flex-col sm:flex-row space-y-8 sm:space-y-0 justify-between">
            <FormSwitch
              ...
              enabled={isShowPassword}
              setEnabled={() => setIsShowPassword(!isShowPassword)}
            ></FormSwitch>
            <Button
              buttonSize="sm"
              text="Log in"
          ...
          <div className="flex flex-col text-center space-y-4 pt-8">
            ...
            <LinkTo to="/forgot-password" className="font-bold">
              Forgot Password?
            </LinkTo>
          </div>
        </form>
